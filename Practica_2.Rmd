...
for(h in nhidden){
  ...
  cat("Los valores de accuracy de entrenamiento y evaluaci?n para ",h," nodos son",history2$metrics$acc[nepochs],"y",  history2$metrics$val_acc[nepochs],"\n")
...
for(i in 1:k){
   ...
   history = model %>% fit( x_train, y_train, batch_size=b_final2, epochs=nepochs, verbose=0,
                            validation_data=list(x_train[eval_index,,,],y_train[eval_index,])
   )
