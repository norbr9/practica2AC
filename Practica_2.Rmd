...
La tecnica L1 de regularización o tambien llamada Lasso Regression, limita la capacidad de la red actuando en su algoritmo de entrenamiento, añadiendo en la función de perdida un coeficiente de penaliación en función de sus valores abosultos.
...
A la entrada de las neuronas se le pasará un producto escalar y usaremos una función de activación RELU y para la salida aplicaremos activación tipo softmax. 


Estos modelos los vamos a construir directamente aplic?ndoles dropout, ya que ha sido la mejor t?cnica para la red MLP. De esta forma nos aseguramos que el overfitting va a estar controlado.

Las capas de convoluci?n se a?aden como paso previo, esto permite reducir las caracter?sticas de las im?genes a las m?s interesantes y nos permiten distinguir unas de otras. Este proceso se lleva a cabo con una matriz de pesos que da lugar a un mapa de activaci?n, que ser? la entrada a nuestra red.

En la primera capa utilizamos 32 filtros, con 3x3 = 9 (tama?o de la matriz) pesos cada uno, lo que nos da 9x32+32 = 320 par?metros, donde 9x32 son los pesos en total de esta capa y lo 32 restantes son los sesgos introducidos por los filtros, en este caso 1 por filtro.

En la segunda capa utilizamos 64 filtros, con 3x3 = 9 pesos por cada profundidad, lo que equivale a 3x32 = 288 pesos por filtro, dando un total de 288x64 = 18432 pesos, y 64 sesgos, uno por filtro.

Por tanto, en total utilizamos 32+64 = 96 filtros, con un total de 320+18432+64 = 18816 pesos.

El mapa de activaci?n es el resultado de recorrer la imagen original con la matriz de pesos, en nuestro caso 3 x 3. Por tanto, nuestro mapa de activaci?n es (a-r+1)x(b-c+1) = (32-3+1)x(32-3+1) = 30x30 = 900, lo que genera un volumen de salida de 30x30x32.

Para la elecci?n de hiper-par?metros a utilizar, usaremos dos bucles para determinar, en el primero, cu?l es el mejor n?mero de nodos ocultos y, en el segundo, cu?l es el tama?o de bach m?s adecuado. Teniendo en cuenta los accuracy que aporte cada hiper-par?metro, se seleccionar? el de mayor valor y se realizar? la crossvalidaci?n con esa configuraci?n.
...
for(h in nhidden){
  ...
  cat("Valores de accuracy de entrenamiento y evaluación para ",h," nodos son:", history2$metrics$acc[nepochs], " y ", history2$metrics$val_acc[nepochs],"\n")
...
plot(y=acctvals2,x=nhidden,ylab="Accuracy",xlab="hidden nodes",main="Accuracy train/test a traves de epochs",col="blue",ylim=c(ymin,1),type="l")
lines(x=nhidden,y=accvvals2,col="red")
```

Como se ha probado con bsize a 128 ahora vamos a ver qué valor de la variable mejoraría el accuracy.
...
Como se puede observar a partir de los resultados obtenidos, la red neuronal obtiene resultados bastante mejores que la MLP. Esto se debe a que es una red m?s compleja y, aunque computacionalmente sea m?s costosa, aprende mejor con el mismo conjunto de datos.
